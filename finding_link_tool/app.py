...
class NewsApp(ctk.CTk):
    def __init__(self):
        ...
        #1. Title App
        self.label = ctk.CTkLabel(self, text="FINDING EVIDENCE FOR VNEXPRESS", font=("Arial", 20, "bold"))
        ...
        self.forward_button = ctk.CTkButton(self.nav_frame, text="▶", width=40, command=self.next_article)
        ...
        self.backward_button = ctk.CTkButton(self.nav_frame, text="◀", width=40, command=self.prev_article)
        ...
        self.link_entry = ctk.CTkEntry(self.subframe1, placeholder_text="Dán link bài báo vào đây", width=350)
        ...
        self.source_combo = ctk.CTkComboBox(
            self.subframe1, 
            values=["thanhnien", "vtv", "tuoitre", "dantri", "vietnamnet", "laodong", "Khác"], 
            width=120,
        ...
        self.custom_source_entry = ctk.CTkEntry(self.subframe1, placeholder_text="Nhập nguồn...", width=120)
        ...
        self.submit_button = ctk.CTkButton(self.subframe2, text="SUBMIT", command=self.add_entry_to_table)
        self.submit_button.pack(side="right", padx=(0, 10))
        self.clear_button = ctk.CTkButton(self.subframe2, text="CLEAR", command=self.clear_input)
        ...
        self.status_label = ctk.CTkLabel(self.saved_frame, text="", text_color="#f39c12")
        self.status_label.pack(side="left")

        #5. Hiện thị các link đã chọn cùng với nguồn của nó:
        self.table_label = ctk.CTkLabel(self, text="📋 Danh sách nguồn đã thêm:", font=("Arial", 12, "bold"))
    ...
    def add_record(self, source, link):
        ...
        # Cột 0: Nguồn
        lbl_source = ctk.CTkLabel(self.table_frame, text=source,text_color="white", font=("Arial", 11))
        ...
        self.save_to_log()
        print(f"Added record: {source} - pending_records now has {len(self.pending_records)} items")
    ...
    def load_data(self):
        ...
        try:
            ...
            if self.total_urls > 0:
                ...
                self.update_status(f"Đã tải {self.total_urls}/{len(raw_data)} links từ {filename}")
            else:
                self.update_ui()
    ...
    def load_records_for_current_article(self):
        ...
        if article_id in log_data.get(self.current_file_type, {}):
            ...
            print(f"Loading from log.json: {article_id} -> {len(saved_records)} records")
    ...
    def save_records_to_file(self):
        ...
        for source, links in records_by_source.items():
            filename = f"{source}_kien_{self.current_file_type}.txt"
